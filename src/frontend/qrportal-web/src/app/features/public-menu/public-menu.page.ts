        ...
        <section class="mx-auto max-w-3xl px-4 py-7 sm:px-6">
          ...
          @for (category of filteredCategories(); track category.name; let categoryIndex = $index) {
            <section class="scroll-mt-22 pt-10" [id]="'category-' + categoryIndex"><h2 class="text-2xl font-black tracking-tight" [style.color]="menu()!.theme.secondaryColor">{{ category.name }}</h2>@if (category.description) { <p class="mt-1 text-sm opacity-55" [style.color]="menu()!.theme.secondaryColor">{{ category.description }}</p> }
              <div class="mt-5 grid gap-4 sm:grid-cols-2">@for (product of category.products; track product.id) { <a [routerLink]="['/p', product.id]" [attr.aria-label]="'Ver detalhes de ' + product.name" class="group block overflow-hidden border border-black/[.06] bg-white shadow-[0_10px_30px_rgba(0,0,0,.05)] transition hover:-translate-y-0.5 hover:shadow-[0_16px_36px_rgba(0,0,0,.09)] focus-visible:outline-3 focus-visible:outline-offset-3" [class]="cardClass" [style.outline-color]="menu()!.theme.primaryColor"><article><div class="aspect-[16/9] overflow-hidden bg-gradient-to-br from-slate-100 to-slate-200">@if (product.imageUrl || product.thumbnailUrl) { <img [src]="product.imageUrl || product.thumbnailUrl" [alt]="product.name" loading="lazy" class="size-full object-cover transition duration-300 group-hover:scale-[1.02]" /> } @else { <div class="grid size-full place-items-center text-4xl">🍴</div> }</div><div class="p-4">@if (product.isFeatured) { <span class="mb-2 inline-flex items-center gap-1 text-[10px] font-black uppercase tracking-widest" [style.color]="menu()!.theme.primaryColor"><svg lucideStar size="12" fill="currentColor"></svg>Destaque</span> }<h3 class="font-black" [style.color]="menu()!.theme.secondaryColor">{{ product.name }}</h3>@if (product.description) { <p class="mt-2 line-clamp-3 text-sm leading-5 text-slate-500">{{ product.description }}</p> }<div class="mt-4 flex items-center justify-between gap-3"><div class="flex items-baseline gap-2">@if (product.promotionalPrice !== null) { <span class="text-lg font-black" [style.color]="menu()!.theme.primaryColor">{{ product.promotionalPrice | currency:'BRL' }}</span><span class="text-xs text-slate-400 line-through">{{ product.price | currency:'BRL' }}</span> } @else { <span class="text-lg font-black" [style.color]="menu()!.theme.primaryColor">{{ product.price | currency:'BRL' }}</span> }</div><span class="inline-flex items-center gap-1 text-xs font-extrabold" [style.color]="menu()!.theme.primaryColor">Ver detalhes <svg lucideArrowRight size="14"></svg></span></div></div></article></a> } @empty { <p class="text-sm text-slate-500">Nenhum produto disponível.</p> }</div>
            </section>
