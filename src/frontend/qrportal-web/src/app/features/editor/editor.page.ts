        ...
        @switch (tab()) {
          ...
          @case ("appearance") {
            <section class="mt-7 grid gap-6 lg:grid-cols-[1fr_390px]">
              <form class="surface rounded-3xl p-6 sm:p-8" [formGroup]="themeForm" (ngSubmit)="saveTheme()"><h2 class="text-xl font-black">Identidade visual</h2><p class="mt-2 text-slate-500">Comece por um preset e ajuste os detalhes.</p><div class="mt-7 grid grid-cols-3 gap-3">@for (preset of presets; track preset.name) { <button type="button" class="rounded-2xl border-2 p-2 text-left" [class]="themeForm.controls.preset.value === preset.name ? 'border-ink' : 'border-slate-200'" (click)="applyPreset(preset)"><span class="block h-14 rounded-xl" [style.background]="preset.primary"></span><span class="mt-2 block text-xs font-extrabold">{{ preset.label }}</span></button> }</div><div class="mt-7 grid gap-4 sm:grid-cols-3"><label class="text-sm font-bold text-slate-600">Primária<input type="color" class="mt-2 h-12 w-full rounded-xl border border-slate-200 p-1" formControlName="primaryColor" /></label><label class="text-sm font-bold text-slate-600">Textos<input type="color" class="mt-2 h-12 w-full rounded-xl border border-slate-200 p-1" formControlName="secondaryColor" /></label><label class="text-sm font-bold text-slate-600">Fundo<input type="color" class="mt-2 h-12 w-full rounded-xl border border-slate-200 p-1" formControlName="backgroundColor" /></label></div><div class="mt-6 grid gap-4 sm:grid-cols-2"><label class="field-label">Estilo dos elementos<select class="field" formControlName="style"><option value="rounded">Arredondado</option><option value="square">Reto</option><option value="pill">Cápsula</option></select></label><label class="field-label">Tipografia<select class="field" formControlName="fontFamily"><option value="sans">Moderna</option><option value="serif">Editorial</option><option value="rounded">Amigável</option></select></label><label class="field-label">Layout dos produtos<select class="field" formControlName="cardLayout"><option value="grid">Grade visual</option><option value="list">Lista compacta</option></select></label><label class="field-label">Encaixe das imagens<select class="field" formControlName="imageStyle"><option value="cover">Preencher</option><option value="contain">Mostrar inteira</option></select></label></div><button class="btn-primary btn-brand mt-7"><svg lucideSave size="18"></svg> Salvar aparência</button></form>
              <app-menu-preview [menu]="menu()!" [theme]="themeForm.getRawValue()" [logoUrl]="store()!.logoUrl" [storeName]="store()!.publicName" />
            </section>
          }
          @case ("qrcode") {
...
export class EditorPage {
  ...
  readonly themeForm = new FormGroup({ preset: new FormControl("green", { nonNullable: true }), primaryColor: new FormControl("#16A34A", { nonNullable: true }), secondaryColor: new FormControl("#0F172A", { nonNullable: true }), backgroundColor: new FormControl("#FFFFFF", { nonNullable: true }), style: new FormControl<"rounded" | "square" | "pill">("rounded", { nonNullable: true }), fontFamily: new FormControl<"sans" | "serif" | "rounded">("sans", { nonNullable: true }), cardLayout: new FormControl<"grid" | "list">("grid", { nonNullable: true }), imageStyle: new FormControl<"cover" | "contain">("cover", { nonNullable: true }) });
