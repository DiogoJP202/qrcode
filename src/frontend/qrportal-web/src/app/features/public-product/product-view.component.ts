      ...
      <div class="mx-auto max-w-5xl">
        ...
        <section class="mt-7 grid gap-7 lg:grid-cols-[minmax(0,1fr)_360px] lg:gap-10">
          ...
          <aside class="self-start rounded-3xl bg-white p-6 text-center text-slate-900 shadow-[0_20px_60px_rgba(15,23,42,.08)] lg:sticky lg:top-8">
            ...
            <div class="mx-auto mt-5 max-w-64 rounded-2xl border border-slate-100 bg-white p-3"><img [src]="qrBaseUrl() + '.svg'" [alt]="'QR Code de ' + details().product.name" class="aspect-square w-full" /></div>
            <div class="mt-5 grid grid-cols-2 gap-3"><a [href]="qrBaseUrl() + '.png?download=true'" class="btn-secondary !px-3 text-xs"><svg lucideDownload size="15"></svg> PNG</a><a [href]="qrBaseUrl() + '.svg?download=true'" class="btn-secondary !px-3 text-xs"><svg lucideDownload size="15"></svg> SVG</a></div>
